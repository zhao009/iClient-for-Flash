package com.supermap.web.iServerJava6R.layerServices
{
	 ...
	 * <p>${iServer6_SetLayerStyleService_Description}</p> 
	 * 
	 */	
	public class SetLayerStyleService extends ServiceBase
	{
		private var _lastResult:SetLayerResult; 
		private var _parameters:SetLayerStyleParameters;
		private var _newResourceID:String;
		private var _responder:IResponder;
		
		/**
		 * ${iServer6_SetLayerStyleService_constructor_String_D} 
		 * @param url ${iServer6_SetLayerStyleService_constructor_String_param_url}
		 * 
		 */		
		public function SetLayerStyleService(url:String)
		{
			super(url);
		}
		...
		private function getPutRequestUrl(parameters:SetLayerStyleParameters):String//获取url
		{ 
		 	var secondUrl:String = "tempLayersSet/" + parameters.resourceID + "/" + parameters.layerName + "@@" + this.getMapName(this.url) + "/style.json?X-RequestEntity-ContentType=application/flex&flexAgent=true&_method=PUT&reference=" + parameters.resourceID + "&elementRemain=true";
			return secondUrl;
		}
		...
		/**
		 * ${iServerJava6R_SetLayerStyleService_method_ProcessAsync_D}.
		 * <p>${iServerJava6R_SetLayerStyleService_method_ProcessAsync_remarks}</p> 
		 * @param parameters ${iServerJava6R_SetLayerStyleService_method_ProcessAsync_param_parameters}
		 * @param responder ${iServerJava6R_SetLayerStyleService_method_ProcessAsync_param_IResponder}
		 * @return ${iServerJava6R_SetLayerStyleService_method_ProcessAsync_return}
		 * 
		 */		
		public function processAsync(parameters:SetLayerStyleParameters, responder:IResponder = null):AsyncToken
		{			
			if (parameters == null)
			{
		...
		private function process(parameters:SetLayerStyleParameters, responder:IResponder):AsyncToken//最后处理
		{
			...
			var reqVar:String =  parameters.style.toJSON();

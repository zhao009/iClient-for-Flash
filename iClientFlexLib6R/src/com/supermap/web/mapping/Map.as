package com.supermap.web.mapping
{
	...
	{	
		...
		sm_internal var dpiCRS:CoordinateReferenceSystem;
		
		//sm_internal var _isPanTrigger:Boolean = false;
		...
		private var _disabledKeys:Array = null;
		...
		private var _marginFactor:Number = 0.382;
		
		private var _restrictedBounds:Rectangle2D = null;
		...
		public function Map()
		{
			...
			_touchAction = new TouchAction(this);
		}
		//////////////////////****end构造函数*****////////////////////////////////
		...
		/**
		 * ${mapping_Map_attribute_restrictedBounds_D}.
		 * <p>${mapping_Map_attribute_restrictedBounds_remsrks_D}</p> 
		 * @see #marginFactor
		 * @return 
		 */	
		public function get restrictedBounds():Rectangle2D
		{
			return _restrictedBounds;
		}

		public function set restrictedBounds(value:Rectangle2D):void
		{
			_restrictedBounds = value;
		}

		/**
		 * ${mapping_Map_attribute_marginFactor_D}.
		 * <p>${mapping_Map_attribute_marginFactor_remarks}</p>
		 * @see #restrictedBounds
		 * @return 
		 */	
...
//			if(value)
		...
		{
			if (this._mapAction)this._mapAction.removeCursor();
		...
		/**
		 * ${mapping_Map_attribute_disabledKeys_D}.
		 * <p>${mapping_Map_attribute_disabledKeys_remarks}</p>
		 * @default null
		 * @return 
		 * 
		 */		
		...
		public function set viewBounds(value:Rectangle2D) : void
		{
			...
			if (value != null)
			{
				...
				{
					var resWidth:Number = this.restrictedBounds.width;
					var resHeight:Number = this.restrictedBounds.height;
					var valWidth:Number = value.width;
					var valHeight:Number = value.height;
					var res:Number = 0;
					//先比较value和用户限制的浏览大小，
					if(resWidth>valWidth && resHeight>valHeight)
					{
						//包含
						if(this.restrictedBounds.containsRect(value))
						{
							//不做修改
						}
							//不包含，但是又大一些，需要调整位置
						...
						{
							this._viewBounds = this.calculateViewBounds(this._viewBounds,this.restrictedBounds); 
						}
					}
					else
					{
						//当用户限制的大小不能完全包含当前的范围时，需要放大知道最接近包含当前窗口大小的范围
						for(var i:int = 0;i<this.resolutions.length;i++)
						{
							if(resWidth/this.resolutions[i]>this.width && resHeight/this.resolutions[i]>this.height)
							{
						...
						this._viewBounds =this.calculateViewBounds(new Rectangle2D(center.x - res*this.width/2, center.y - res*this.height/2,
							center.x + res*this.width/2, center.y + res*this.height/2),this.restrictedBounds); 
					}
				}
		...
		//将 当前的ViewBounds移进restrictedBounds里面，
		private function calculateViewBounds(oldViewBounds:Rectangle2D,restrictedBounds:Rectangle2D):Rectangle2D
		{
			var left:Number = oldViewBounds.left;
			var bottom:Number = oldViewBounds.bottom;
			var right:Number = oldViewBounds.right;
			var top:Number = oldViewBounds.top;
			
			if(left < this.restrictedBounds.left)
			{
				right = right + (this.restrictedBounds.left - left);
				left = this.restrictedBounds.left;
			}
			
			if(right > this.restrictedBounds.right)
			{
				left = left - (right - this.restrictedBounds.right) ;
				right = this.restrictedBounds.right;
			}
			
			if(bottom < this.restrictedBounds.bottom)
			{
				top = top + (this.restrictedBounds.bottom - bottom);
				bottom = this.restrictedBounds.bottom;
			}
			
			if(top > this.restrictedBounds.top)
			{
				bottom = bottom - (top -this.restrictedBounds.top);
				top = this.restrictedBounds.top;
			}
		...
		override protected function createChildren() : void
		{
			...
			if (!UIComponentGlobals.designMode)
			{
				...
				this._screenContainer.addElement(this._infoWindow.infoWindowShadow);
		...
		sm_internal function mapToContainerX(mapX:Number) : Number
		{
			//			if (this.oldViewBounds)
			//			{
			//				return (mapX - this.oldViewBounds.left) * width / this.oldViewBounds.width + this.layerContainer.scrollRect.x;
			//			}
			if (this._viewBounds)
			{
...
//				return (mapX - this._viewBounds.left) * width / this._viewBounds.width + this.layerContainer.scrollRect.x;
		...
		sm_internal function mapToContainerY(mapY:Number) : Number
		{
			//			if (this.oldViewBounds)
			//			{
			//				return (this.oldViewBounds.top - mapY) * height / this.oldViewBounds.height + this.layerContainer.scrollRect.y;
			//			}
			if (this._viewBounds)
			{
...
//				return (this._viewBounds.top - mapY) * height / this._viewBounds.height + this.layerContainer.scrollRect.y;
		...
		private function dbClickHandler(event:MouseEvent) : void
		{
			if(this._doubleClickZoomEnabled)
			{
				...
				{					
					...
					if(isZoomIn)
					{
						if(this.resolutions != null && currentLevel >= (this.resolutions.length - 1))
						{
							return;
						}
					}
					...
					{
						if(this.resolutions != null && currentLevel <= 0)
						{
							return;
						}
					}
		...
		override protected function keyDownHandler(event:KeyboardEvent):void
		{
			if(this._keyboardNavigationEnabled)
			{
				...
				var isKeyDisabled:Boolean = this.isKeyInDisabledKeys(event.keyCode);
		...
		private function isKeyInDisabledKeys(key:uint) : Boolean
		{	
			var isIn:Boolean = false;
			if(this._disabledKeys)
			{
				for(var i:Number = 0, len:Number = this._disabledKeys.length; i < len; i++ )
				{
					if(key == this._disabledKeys[i])
					{
						isIn = true;
						break;
					}
		...
		{
			if (this._creationComplete && this._baseLayerLoaded)
			{
				//this._mapAction.addMapListeners();
				addEventListener(ViewBoundsEvent.VIEW_BOUNDS_CHANGE,this.viewBoundsChangedHandler, false);
				addEventListener(MouseEvent.DOUBLE_CLICK,this.dbClickHandler, false);
		...
		//计算map的x方向是否可移动
		sm_internal function panXEnable(xFrom:Number, xTo:Number):Boolean
		{
			if(null == _restrictedBounds)
			{
				return true;
			}
			if(((xFrom - xTo)>0 && (this.oldViewBounds.right + (xFrom - xTo)*this.resolution)>this._restrictedBounds.right) || ((xFrom - xTo)<0 && (this.oldViewBounds.left - (xTo - xFrom)*this.resolution)<this._restrictedBounds.left))
			{
				return false;
			}
			return true;
		...
		sm_internal function panYEnable(yFrom:Number, yTo:Number):Boolean
		{
			if(null == _restrictedBounds)
			{
				return true;
			}
			if(((yFrom - yTo) > 0 && (this.oldViewBounds.bottom - (yFrom - yTo)*this.resolution) < this._restrictedBounds.bottom) || ((yFrom - yTo) < 0 && (this.oldViewBounds.top + (yTo - yFrom)*this.resolution) > this._restrictedBounds.top))
			{
				return false;
			}
			return true;
		...
		private function baseLayerLoadHandler(event:LayerEvent) : void
		{
			...
			{
				this._viewBounds = this.bounds.clone();
			}
		...
		private function layerLoadHandler(event:LayerEvent) : void
		{
			...
			{
				layer.setMap(this);
				//layer.dispatchEvent(new LayerEvent(LayerEvent.LOADED, layer));
			}
			if(layer.isScaleCentric)
			{
				this.scaleConvertion();
		...
		{
			var oldCrs:CoordinateReferenceSystem;
			this.dpiCRS = null;
			var _dpi:Number = this.dpi;
			if(this.CRS)
				oldCrs = this.CRS.clone();
			else if(this.dpiCRS)
				this._CRS = dpiCRS;
			if(!isNaN(_dpi) && _dpi != 0)
			{

package com.supermap.web.mapping
{
	...
	public class UTFGridLayer extends TiledDynamicRESTLayer
	{
		...
		/**
		 * ${mapping_UTFGridLayer_attribute_filter_D}
		 * @return
		 *
		 */
		...
		override protected function removeMapListeners() : void
		{
			...
			this.removeUTFGridMouseListeners();
		}
		
		// 移除  UTFGrid 鼠标事件
		private function removeUTFGridMouseListeners() : void
		{
			if (this.map)
			{
				if (UTFGridMouseClickHandler != null)
				{
					this.map.removeEventListener(MouseEvent.CLICK, clickHandler);
				}
				if (UTFGridMouseMoveHandler != null)
				{
					this.map.removeEventListener(MouseEvent.MOUSE_MOVE, moveHandler);
				}
			}
		}
		...
		protected function clickHandler(event:MouseEvent):void
		{
			...
			var screenX:Number = (event.currentTarget as Map).mouseX;
			var screenY:Number = (event.currentTarget as Map).mouseY;
			var data:Object = getDataFromMousePostion(screenX, screenY);
			this.UTFGridMouseClickHandler(data, screenX, screenY);
		}
		/**
		 * @inheritDoc 
		 * 
		...
		protected function moveHandler(event:MouseEvent):void
		{
			...
			var screenY:Number = (event.currentTarget as Map).mouseY;
			var data:Object = getDataFromMousePostion(screenX, screenY);
			this.UTFGridMouseMoveHandler(data, screenX, screenY);
		}
		/**
		 * @inheritDoc 
		 * 
		...
		override protected function getTileURL(row:int, col:int, resolution:Number):URLRequest
		{
			...
			serverURL = encodeURI(this.url) + "utfGrid.json?scale=" + scale + "&x=" + col + "&y=" + row + "&width=" + this.tileSize + "&height=" + this.tileSize + "&pixCell=" + this.pixcell;
			if(this.layerName)
			{
				serverURL += "&layerNames=" + encodeURIComponent("[\""+this.layerName+"\"]");
			}
			if(this.filter)
			{
				serverURL += "&filters=" + encodeURIComponent("[\""+this.filter+"\"]");
			}
			//管理Credential

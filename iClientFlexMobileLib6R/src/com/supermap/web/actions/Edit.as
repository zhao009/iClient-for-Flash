package com.supermap.web.actions
{
	...
	/**
	 * ${actions_Edit_Title}.
	 * <p>${actions_Edit_Description}</p>
	 * 
	 */
	public class Edit extends DrawAction
	{
		...
		/**
		 * ${actions_Edit_constructor_D} 
		 * @param map ${actions_Edit_constructor_param_map} 
		 * @param featureLayer ${actions_Edit_constructor_param_layer} 
		 * 
		 */
...
//			this._editLayer.removeEventListener(FeatureLayerEvent.FEATURE_ADD, addFeatureHandler);
		...
		private function onEdittingFeatureMove(event:MouseEvent):void
		{
			...
			if(this._edittingFeature.geometry is GeoPoint)
			{
				return;
			}
			else if(this._edittingFeature.geometry is GeoLine)
			{
			...
			else // if(this._edittingFeature.geometry is GeoRegion)
			{
				...
				for(var m:int = 0; m < this._edittingFeaturePartCount; m++)
				{
					var regionPoint2Ds:Array = geoRegion.getPart(m);
					var regionPointsLength:int = regionPoint2Ds.length;
					for(var n:int = 0; n < regionPointsLength; n++)
					{
						var regionP:Point2D = regionPoint2Ds[n];
						regionPoint2Ds.splice(n, 1, new Point2D(regionP.x - detaX, regionP.y - detaY));
					}
					geoRegion.removePart(m);
					geoRegion.insertPart(m, regionPoint2Ds);
				}
...
//				this._editLayer.moveToTop(currentPartVertexFeatures[1]);
		...
		private function updateCurrentPartFeature(point2Ds:Array, partIndex:int):void
		{
...
////			this._features.addItem(addedFeature);
...
//			this._edittingFeature.attributes = null;
			this._draggedVertex = null;
			if(!this._isEndEditting)
				this._isEndEditting = true;
			this._isRemoveFeature = false;

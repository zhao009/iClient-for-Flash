package com.supermap.web.core.styles
{
	...
	public class PredefinedMarkerStyle extends MarkerStyle
	{
		...
		private function drawPointCircle(sprite:Sprite, size:Number, half:Number, point:GeoPoint) : void
		{
			...
			if(this._border)
			{
				this._stroke.color = this._border.color;				
				this._stroke.alpha = this._border.alpha;
				this._stroke.weight = this._border.weight;
				if(this._border.symbol == PredefinedLineStyle.SYMBOL_SOLID)
				{				
//					if(this._angle)
//					{
//						this.traceRotationCircle(sprite, half, null, false, this._stroke, point);
//					}
//					else
//					{
...
//					{
						...
						StyleUtil.drawStyledGeoLine(sprite, ary, this._stroke, this.pattern);				
//					}
				}
			}			
			else
...
//				StyleUtil.drawStyledGeoLine(sprite, arrCirclePoints, this._stroke, this.pattern);
//			}
		...
		{ 			
			...
			if(this._border)
			{
				...
				if(this._border.symbol==PredefinedLineStyle.SYMBOL_SOLID)
				{
					if (this.angle)
					{
						this.traceRotationStar(sprite, this._size,this._half, null, false, this._stroke, point);
					}
					else
					{
						this._stroke.apply(sprite.graphics, null, null);
				...
				{	
					if(this.angle)
					{
						this.traceRotationStar(sprite, this._size, this._half, ptsStar, true, this._stroke, point);
					}
					else
					{					
						this.traceStar(sprite, half, ptsStar, half, half);	
						sprite.graphics.endFill();
						StyleUtil.drawStyledGeoLine(sprite, ptsStar, this._stroke, this.pattern);
			...
			else
			{
				...
				if(this.angle)
				{
					this.traceRotationStar(sprite, this._size, this._half, ptsStar, false, this._stroke, point);
				}
				else
				{
					this.traceStar(sprite, half, ptsStar, half, half);						
		...
		private function drawPointSquare(sprite:Sprite, size:Number, half:Number, point:GeoPoint) : void
		{
			...
			if (this._border)
			{
				...
				{					
					if (this.angle)
					{
						this.traceRotationSquare(sprite, size, half, null, false, this._stroke, point);
					}
					else
					{
						this._stroke.apply(sprite.graphics, null, null);
				...
				else
				{
					...
					if (this.angle)
					{
						this.traceRotationSquare(sprite, size, half, ptsSquare, true, this._stroke, point);
					}
					else
					{						
						...
						sprite.graphics.drawRect(0, 0, size, size);
						sprite.graphics.endFill();
						StyleUtil.drawStyledGeoLine(sprite, ptsSquare, this._stroke, this.pattern);
					}
				}
			}
			else
		...
		private function drawPointTriangle(sprite:Sprite, size:Number, half:Number, point:GeoPoint) : void
		{
			...
			if (this._border)
			{
				...
				{				
					if (this.angle)
					{
						this.traceRotationTriangle(sprite, size, half, null, false, this._stroke, point);
					}
					else
					{
						this._stroke.apply(sprite.graphics, null, null);
				...
				else
				{
					...
					if (this.angle)
					{						
						this.traceRotationTriangle(sprite, size, half, ptsTriangle, true, this._stroke, point);
					}
					else
					{						
						this.traceTriangle(sprite.graphics, half, half, half);
						...
						StyleUtil.drawStyledGeoLine(sprite, ptsTriangle, this._stroke, this.pattern);
...
//			sprite.height = this.size * 2;			
			...
			if (this._border)
			{
				...
				if (this._border.symbol == PredefinedLineStyle.SYMBOL_SOLID)
				{
					if(this.angle)
					{
						this.traceRotationSector(sprite, size, half, ptsSector, false, this._stroke, point);
					}
					else
					{
						this._stroke.apply(sprite.graphics, null, null);
				...
				else
				{
					if(this.angle)
					{
						this.traceRotationSector(sprite, size, half, ptsSector, true, this._stroke, point);
					}
					else
					{
						sprite.graphics.beginFill(this.color, this.alpha);
						...
						sprite.graphics.endFill();					
						StyleUtil.drawStyledGeoLine(sprite, ptsSector, this._stroke, this.pattern);
					}
		...
		{			
			...
			var pts:Array = null;
			sprite.graphics.beginFill(this._color, this._alpha);
			this._stroke.color = this._color;
			this._stroke.alpha = this._alpha;
			if (this._border)
			{
				this._stroke.weight = this._border.weight;
				if (this._border.symbol == PredefinedLineStyle.SYMBOL_SOLID)
				{					
					if (this.angle)
					{
						this.traceRotationX(sprite, size, half, null, false, this._stroke, point);
					}
					else
				...
				else
				{
					...
					if (this.angle)
					{
						this.traceRotationX(sprite, size, half, ptsX, true, this._stroke, point);
					}
					else
					{
						...
						{	
							StyleUtil.drawStyledGeoLine(sprite,pts,this._stroke, this.pattern);
						}
					}
				}
			}
		...
		private function drawPointDiamond(sprite:Sprite,size:Number,half:Number,point:GeoPoint) : void
		{
			...
			if (this._border)
			{
				...
				{					
					if (this.angle)
					{
						this.traceRotationDiamond(sprite, size, half, null, false, this._stroke, point);
					}
					else
					{
						this._stroke.apply(sprite.graphics, null, null);
				...
				else
				{
					...
					if (this.angle)
					{
						this.traceRotationDiamond(sprite, size, half, ptsDiamond, true, this._stroke, point);
					}
					else
					{
						var AToR:Number = Math.PI / 180;
						...
						this.traceDiamond(sprite.graphics, half, half, half);
						sprite.graphics.endFill();
						StyleUtil.drawStyledGeoLine(sprite, ptsDiamond, this._stroke, this.pattern);
					}
				}
			}
			else
		...
		private function drawOther(sprite:Sprite, geometry:Geometry, map:Map) : void
		{
			...
			if(geometry is GeoRegion)
			{
				...
				//面为闭合环路，最后的节点和第一个节点重合，避免重绘
				for(k; k < regionPartNum - 1; k++)
				{		
					geoRegionPath = geoRegion.getPart(k);
					var regionPntCount:int = geoRegionPath.length;
					if (regionPntCount > 1)
					{
						for(var p:int = 0;p < regionPntCount; p++)
						{
...
//						StyleUtil.drawStyledGeoLine(customSprite, pts, this._stroke, this.pattern);
//						customSprite.graphics.endFill();
//					}
				}
			}
...
//				{
					...
					customSprite.y = ptY;
					customSprite.rotation = angle;
					this.traceMultipointX(map, customSprite, pt, this._stroke, 0, 0, true);
//					continue;
//				}
//				this.traceMultipointX(map, sprite, pt, this._stroke, ptX, ptY, false);
			}

package com.supermap.web.mapping
{
	...
	public class GraphicsLayer extends Layer
	{
		...
		public function GraphicsLayer()
		{
			...
			this.addEventListener(MouseEvent.CLICK, behindLayerClickHandler);
		}
		...
		private function behindLayerClickHandler(event:MouseEvent):void
		{
			if (graphicClickHandler != null)
			{
				...
				//屏幕上的像素坐标
				var mousePoint:Point = new Point(tar.mouseX - parentContainer.scrollRect.x, tar.mouseY - parentContainer.scrollRect.y);
				...
				//获取鼠标点对应的颜色值
				var color1:uint = this._bmd.getPixel32(mousePoint.x, mousePoint.y);
				var alphat:uint = (color1 >> 24) & 0xFF;
				if(alphat != 255)
					return;
				
				var color:uint = this._bmd.getPixel(mousePoint.x, mousePoint.y);
				var chex:String = color.toString(16);
				var nc:int = parseInt(chex, 16);
				var currentF1:Graphic = _fDictionary[nc] as Graphic;
				if(currentF1)
				{
					var containedPoint:Point2D = this.map.screenToMap(new Point(mousePoint.x, mousePoint.y));
					...
					if(currentF1.geometry is GeoPoint)
					{
						...
						var point:GeoPoint = currentF1.geometry as GeoPoint;
						if(style.symbol == "icon")
						{
							var geo:GeoRegion = this.getIconBounds(point.tempIconRec);
//							this.add([new Graphic(geo)]);
						...
						else
						{
							...
							var screenPoint:Point = this.map.mapToScreen(new Point2D(point.x,point.y));
